...
if(userInfo==null && $.cookieHelper('autoLoginInfo')){
		 window.location = JYUserUrl('autoLogin' ,'', {target: window.location.href}, 'user.91160.com');		
}
//浮动窗
...
//			&& _validate(_nycheckcode)){
	...
	$("#search_key").autocomplete("/main/ajaxsearch/no_city-"+noCity+'.html', {
		dataType: "json",
		minChars: 0,
		width: 345,
		matchContains: "word",
		...
		formatItem: function(row) {
			if(row.type==1) {
				if(row.sort==-1) {
					return '<i class="icon-hospital"></i>  搜索“'+ row.name+'”相关的医院&nbsp;&gt;&gt;';
				} else {
					return row.name;
				}
			}
			if(row.type==2) {
				if(row.sort==-1) {
					return '<i class="icon-doctor"></i>  搜索“'+ row.name+'”相关的医生&nbsp;&gt;&gt;';
				} else {
					return row.name;
				}
			}
		...
		if(data.type==1 && data.id=='search_units') {
			$("#serch_type").val(data.type);
			search();
		} else if (data.type==2 && data.id=='search_docs'){
			$("#serch_type").val(data.type);
			search();
		}else{
			if(data.type==1) {
				location.href = JYUrl('unit','show',{uid:data.id,skey:data.skey});
			}else if(data.type==2){
				location.href = JYUrl('doc','show',{depid:data.depid,docid:data.id});
			} else {
				search();
			}
		}
...
function JYUrl(cName,aName,args, domain){
    ...
    var sUrl = "http://"+hostName+"/"+cName+"/"+aName;
    if(args!=undefined){
            for(k in args){
                    sUrl += "/"+k+"-"+args[k];
            }
